...
fun LikeBurstEffect(
    ...
    startRadiusFraction: Float = 0.30f,
    endRadiusFraction: Float = 0.85f,
    ...
    Canvas(modifier = modifier) {
        ...
        val startRadius = minDim * startRadiusFraction
        val endRadius = minDim * endRadiusFraction

        val ringRadius = lerp(startRadius, endRadius, p)
        val ringAlpha = (1f - p).coerceIn(0f, 1f)
        drawCircle(
            color = Color.White.copy(alpha = ringAlpha * 0.35f),
            radius = ringRadius,
            center = center,
            style = Stroke(width = minDim * 0.06f * (1f - p) + 1f)
        )

        val particleCount = 12
        val baseRadius = minDim * 0.08f

        for (i in 0 until particleCount) {
            val angle = (i.toFloat() / particleCount) * (Math.PI.toFloat() * 2f)
            val dist = lerp(startRadius, endRadius, p)
        ...
        for (i in 0 until dotCount) {
            ...
            val dist = lerp(startRadius * 1.05f, endRadius, p)
            val x = center.x + cos(angle) * dist
            val y = center.y + sin(angle) * dist
